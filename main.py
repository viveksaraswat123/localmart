...
def role_required(allowed_roles: list[str]):
    def check(current_user: models.User = Depends(get_current_user)):
...
async def create_product(
    ...
    if len(image) < 3:
        raise HTTPException(400, "At least 3 images are required")
    ...
    if "grocery" in category.lower() and not expiry_date:
        raise HTTPException(400, "Grocery products must include an expiry date")

    contents = await thumb.read()
...
def update_product(
    ...
    if current_user.role != "admin" and product.seller_id != current_user.id:
        raise HTTPException(403, "You are not authorized to edit this product")

    for field in ["name", "description", "price", "quantity", "category", "expiry_date"]:
        value = getattr(product_in, field, None)
...
def delete_product(
    ...
    if not product:
        raise HTTPException(404, "Product not found")
    if current_user.role != "admin" and product.seller_id != current_user.id:
        raise HTTPException(403, "You are not authorized to delete this product")
    db.delete(product)
    db.commit()
...
def update_service(
    ...
    for field in ["type", "name", "cost", "location"]:
        value = getattr(service_in, field, None)
        if value is not None:
